-- =====================================================================
-- Scout: catch-up migration for the production database.
-- Paste the WHOLE file into the Supabase SQL editor and run it.
-- Every statement is idempotent, so running it twice is harmless.
--
-- What it fixes, measured against the live database on 2026-08-20:
--   1. shared_projects.open_to_workspace was missing. PostgREST fails the
--      whole query on one unknown column, so every team query returned
--      nothing and the app read that as "no shared projects". Teammates'
--      finds could never sync.
--   2. workspaces.location was missing, which emptied the company list and
--      hid the company switcher.
--   3. target_contacts did not exist at all, so the anti-overexposure cap
--      (the thing that stops several users blasting one contact) was
--      failing open and capping nobody.
-- =====================================================================

-- ============ PART 1 of 2: teams (from supabase/teams.sql) ============
...
-- ============ PART 2 of 2: exposure cap (from supabase/exposure.sql) ============
