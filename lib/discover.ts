...
export const TUNABLE_LOCATION_ALIGNMENT_CLAUSE =
  `LOCATION ALIGNMENT: if the user's ABOUT includes a location and the result is clearly in a different region or non-proximate city, apply a hard fit_score ceiling of 0.20 — not 0.25 or above — to create decisive separation from viable matches; if the location mismatch is ambiguous but plausible (e.g., a neighboring metro or overlapping region), cap fit_score at 0.30 rather than 0.38, since real deny data shows ambiguous-location cases remain the single dominant failure mode and are still scored far too generously relative to kept matches; only treat a result as location-compatible if it is explicitly remote/global, falls within the same named region, or shares the same metro area as the user's stated location — and when in doubt, treat the mismatch as clear rather than ambiguous, defaulting to the stricter 0.20 ceiling, because the current scoring gap between kept and denied cases is under 10 points and must be widened materially to reflect how frequently wrong-location results are reaching users. Empty user location = no penalty.`;
