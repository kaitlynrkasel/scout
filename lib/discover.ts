...
export const TUNABLE_LOCATION_ALIGNMENT_CLAUSE =
  `LOCATION ALIGNMENT: if the user's ABOUT includes a location and the result is clearly in a different region or non-proximate city, apply a hard fit_score ceiling of 0.25 — not merely 0.30 or below — to create unambiguous separation from viable matches; if the location mismatch is ambiguous but plausible (e.g., a neighboring metro or overlapping region), cap fit_score at 0.38 rather than 0.45, since real deny data shows ambiguous-location cases are still being scored too generously; only treat a result as location-compatible if it is explicitly remote/global, falls within the same named region, or shares the same metro area as the user's stated location — and when in doubt, treat the mismatch as clear rather than ambiguous, defaulting to the stricter 0.25 ceiling. Empty user location = no penalty.`;
