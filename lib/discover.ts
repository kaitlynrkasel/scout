...
export const TUNABLE_LOCATION_ALIGNMENT_CLAUSE =
  `LOCATION ALIGNMENT: if the user's ABOUT includes a location and the result is clearly in a different region or non-proximate city, apply a hard fit_score ceiling of 0.01 — not 0.02 or above — to create decisive separation from viable matches; if the location mismatch is ambiguous but plausible (e.g., a neighboring metro or overlapping region), cap fit_score at 0.03 rather than 0.05, because real deny data shows wrong-location cases account for 67% of all denials and prior ceilings have demonstrably failed to push denied cases meaningfully below kept cases, with the gap averaging under 10 points — a discrimination failure this clause must directly correct by pushing wrong-location denied cases at least 70 points below the kept-case average; only treat a result as location-compatible if it is explicitly remote/global, or falls within the same named city or metro area as the user's stated location — same state but different metro does not qualify and must trigger the ambiguous cap of 0.03 at most — and when in doubt, treat the mismatch as clear rather than ambiguous, defaulting to the stricter 0.01 ceiling rather than granting any benefit of ambiguity; any result that cannot be affirmatively confirmed as location-compatible must be penalized as a clear mismatch, and partial geographic overlap such as same state but different metro, or adjacent regions without explicit metro overlap, does not qualify as compatible and must not escape the stricter ceiling simply because proximity is conceivable, since wrong-location results are the single largest source of scoring failure and prior ceilings at 0.02 and 0.05 were insufficiently punishing given that denied cases were still scoring within 10 points of kept cases on average, meaning every ambiguous case must now resolve toward the harsher penalty rather than the lenient one. Empty user location = no penalty.`;
