...
export const TUNABLE_LOCATION_ALIGNMENT_CLAUSE =
  `LOCATION ALIGNMENT: if the user's ABOUT includes a location and the result is clearly in a different region, different state, or any non-proximate city, apply a hard fit_score ceiling of 0.01 — never 0.02 or above — to create decisive separation from viable matches; if the location mismatch is genuinely ambiguous but plausible (strictly limited to confirmed neighboring metros with documented overlap, not merely adjacent states or same-state different-metro cases), cap fit_score at 0.02 rather than 0.03, but only if the neighboring-metro overlap can be affirmatively confirmed from explicit text in the result — if it cannot be confirmed, default immediately to 0.01 without exception; real deny data shows wrong-location cases account for 67% of all denials and every prior ceiling failed to push denied cases meaningfully below kept cases, with the gap averaging under 10 points — this clause must directly correct that discrimination failure by pushing every wrong-location denied case to a raw fit_score of 0.01 before any other scoring logic is applied, overriding all positive signals including strong role fit, seniority match, or industry alignment, so that the final fit_score cannot exceed 0.01 regardless of how well the result scores on non-location dimensions; only treat a result as location-compatible if it is explicitly and unambiguously labeled remote, fully distributed, or global in the result text itself, or if it falls within the exact same named city or confirmed metro area as the user's stated location — same state but different metro is always a clear mismatch triggering the 0.01 ceiling, same country but different state is always a clear mismatch triggering the 0.01 ceiling, and any result whose city or location is unnamed, inferred, implied by headquarters address, or only partially stated must be penalized as a clear mismatch at 0.01 rather than granted any proximity benefit of the doubt; partial geographic overlap of any kind does not qualify as compatible, vague language such as "serving the region" or "offices nationwide" does not constitute a remote or global designation, and when in doubt the 0.01 ceiling applies unconditionally and immediately. Empty user location = no penalty.`;
