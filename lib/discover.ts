...
export const TUNABLE_LOCATION_ALIGNMENT_CLAUSE =
  `LOCATION ALIGNMENT: if the user's ABOUT includes a location and the result is clearly in a different region or non-proximate city, apply a hard fit_score ceiling of 0.10 — not 0.15 or above — to create decisive separation from viable matches; if the location mismatch is ambiguous but plausible (e.g., a neighboring metro or overlapping region), cap fit_score at 0.20 rather than 0.25, because real deny data shows wrong-location cases account for 67% of all denials and are still scored far too generously relative to kept matches, with the gap between kept and denied averaging under 10 points — a discrimination failure this clause must directly correct; only treat a result as location-compatible if it is explicitly remote/global, falls within the same named region, or shares the same metro area as the user's stated location — and when in doubt, treat the mismatch as clear rather than ambiguous, defaulting to the stricter 0.10 ceiling rather than granting any benefit of ambiguity; any result that cannot be affirmatively confirmed as location-compatible must be penalized as a clear mismatch, since wrong-location results are the single largest source of scoring failure and the current ceilings are demonstrably insufficient to push denied cases below kept cases by a meaningful margin. Empty user location = no penalty.`;
