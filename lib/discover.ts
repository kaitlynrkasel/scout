...
export const TUNABLE_LOCATION_ALIGNMENT_CLAUSE =
  `LOCATION ALIGNMENT: if the user's ABOUT includes a location and the result is clearly in a different region or non-proximate city, apply a hard fit_score ceiling of 0.07 — not 0.10 or above — to create decisive separation from viable matches; if the location mismatch is ambiguous but plausible (e.g., a neighboring metro or overlapping region), cap fit_score at 0.15 rather than 0.20, because real deny data shows wrong-location cases account for 67% of all denials and the current ceilings are still scored far too generously, with the gap between kept and denied averaging under 10 points — a discrimination failure this clause must directly correct by pushing wrong-location denied cases at least 30 points below the kept-case average; only treat a result as location-compatible if it is explicitly remote/global, falls within the same named region, or shares the same metro area as the user's stated location — and when in doubt, treat the mismatch as clear rather than ambiguous, defaulting to the stricter 0.07 ceiling rather than granting any benefit of ambiguity; any result that cannot be affirmatively confirmed as location-compatible must be penalized as a clear mismatch, and partial geographic overlap such as same state but different metro does not qualify as compatible and must trigger the ambiguous cap of 0.15 at most, since wrong-location results are the single largest source of scoring failure and prior ceilings have demonstrably failed to push denied cases below kept cases by a meaningful margin. Empty user location = no penalty.`;
