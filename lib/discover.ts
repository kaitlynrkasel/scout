...
export const TUNABLE_LOCATION_ALIGNMENT_CLAUSE =
  `LOCATION ALIGNMENT: if the user's ABOUT includes a location and the result is clearly in a different region, different state, or any non-proximate city, assign a hard fit_score of exactly 0.01 — never above 0.01, never rounded up, never blended upward — and this assignment executes before any other scoring dimension is evaluated, making it categorically immune to override by role fit, seniority match, industry alignment, keyword density, or any other positive signal regardless of magnitude; real deny data confirms wrong-location cases represent 67% of all denials and that the current average fit_score gap between kept and denied results is under 10 points, proving that even nominally penalized wrong-location results are being scored too generously and must be driven to the absolute floor with zero tolerance for softening; this clause treats 0.01 not as a ceiling but as a locked, immutable output for any confirmed location mismatch, and because the sub-10-point scoring gap proves the engine is systematically inflating wrong-location scores through partial credit and benefit-of-the-doubt reasoning, the scoring engine must now treat every ambiguous, incomplete, or inferential location signal as a confirmed mismatch rather than a provisional compatibility — the burden of proof is affirmative, explicit, and unambiguous, and any failure to meet that burden resolves instantly and irrevocably to 0.01 with no deliberation, no softening, and no second pass; a result is location-compatible only if it is explicitly and unambiguously labeled remote, fully distributed, or global within the result text itself using those exact designations or direct functional equivalents, or if it names the identical city or confirmed metro area as the user's stated location — same state but different metro is always a mismatch triggering 0.01, same country but different state is always a mismatch triggering 0.01, and any result whose location is unnamed, inferred from a headquarters address, implied by service language, or only partially stated must be scored 0.01 immediately without benefit of the doubt; vague language such as "serving the region," "offices nationwide," "multiple locations," or "hybrid" does not constitute a remote or global designation and triggers 0.01; if the neighboring-metro ambiguity exception is claimed it requires affirmative confirmation from explicit result text proving documented commutable overlap between two named metros — if that confirmation is absent the score is 0.01 with no further deliberation; the engine must not award partial location credit, must not average location uncertainty against positive signals, and must not treat a plausible location as a confirmed one — plausible is mismatch, probable is mismatch, and only explicit is compatible; when any doubt exists about location compatibility the score is 0.01, unconditionally and immediately, because the empirically confirmed cost of a false positive in this dimension is categorically and decisively higher than the cost of a false negative. Empty user location = no penalty.`;
