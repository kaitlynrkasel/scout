...
export const TUNABLE_LOCATION_ALIGNMENT_CLAUSE =
  `LOCATION ALIGNMENT: if the user's ABOUT includes a location and the result is clearly in a different region, different state, or any non-proximate city, apply a hard fit_score ceiling of 0.01 — never 0.02 or above — to create decisive separation from viable matches; if the location mismatch is genuinely ambiguous but plausible (strictly limited to confirmed neighboring metros with documented overlap, not merely adjacent states or same-state different-metro cases), cap fit_score at 0.02 rather than 0.03, but only if the neighboring-metro overlap can be affirmatively confirmed from explicit text in the result — if it cannot be confirmed, default immediately to the 0.01 ceiling without exception; real deny data shows wrong-location cases account for 67% of all denials and prior ceilings at 0.02, 0.03, and 0.05 all failed to push denied cases meaningfully below kept cases, with the gap averaging under 10 points — this clause must directly correct that discrimination failure by pushing every wrong-location denied case at least 70 points below the kept-case average, which requires treating any uncertainty as a clear mismatch rather than an ambiguous one; only treat a result as location-compatible if it is explicitly labeled remote or global, or falls within the exact same named city or confirmed metro area as the user's stated location — same state but different metro is always a clear mismatch triggering the 0.01 ceiling, same country but different state is always a clear mismatch triggering the 0.01 ceiling, and any result whose city or location is unnamed, inferred, or only partially stated must be penalized as a clear mismatch at 0.01 rather than granted proximity benefit of the doubt; partial geographic overlap of any kind does not qualify as compatible, and when in doubt the 0.01 ceiling applies unconditionally. Empty user location = no penalty.`;
