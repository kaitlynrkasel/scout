...
import { outlookConversationsWithReplies } from "@/lib/outlook";

export const runtime = "nodejs";
export const maxDuration = 30;

// Check which of the user's tracked Outlook conversations have a reply. Reads
// only From headers + isDraft, never message bodies. Returns the find ids that
// replied. Mirrors /api/gmail/replies.
  ...
  try {
    ...
    if (e?.needsReconnect) {
      return NextResponse.json(
        {
          error:
            "Reply tracking needs mailbox read access. Disconnect and reconnect Outlook in your Profile to enable it.",
          needsReconnect: true,
        },
        { status: 403 }
      );
